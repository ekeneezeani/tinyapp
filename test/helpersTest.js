...
describe("getUrlByUser", function() {
  it("it should return all url objects a user created", function() {
    const user = getUrlByUser("2x4R3H", urlDatabase);
    const expected = {
      B4n8Q1: {
        id: "B4n8Q1",
        longURL: "http://www.google.com",
  ...
  it("it should return null if user id does not exist", function() {
    const user = getUrlByUser("2x4R4i", urlDatabase);
    const expected = null;
    assert.equal(user,expected);
  });
